...
df["Genre"].str.split(",").apply(lambda genres: frozenset(genre.strip() for genre in genres if genre)).tolist()
df["Actors"] = df["Actors"].astype(str)
df["Actors"].str.split(",").apply(lambda actors: frozenset(actor.strip() for actor in actors if actor)).tolist()
df["Director"] = df["Director"].astype(str)
df["Director"].str.split(",").apply(lambda dirs: frozenset(dir.strip() for dir in dirs if dir)).tolist()
...
rules = association_rules(
    transactions, frequent_itemsets, min_confidence=0.6
)
